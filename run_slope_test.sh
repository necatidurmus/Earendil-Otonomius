#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════════
#  run_slope_test.sh  —  Eğimli Arazi Travers Test Başlatıcı
#  Leo Rover: Eğim travers kabiliyeti testi (5°-30° eğimler)
#
#  Tüm ayarlar sim_config.yaml'dan okunur.
#  Argümanlar config'i override eder.
#
#  Kullanım:
#    ./run_slope_test.sh                    # Eğimli arazi dünyası
#    ./run_slope_test.sh --rviz             # RViz ile görselleştir
#    ./run_slope_test.sh --timeout 600      # 10 dakika timeout
#    ./run_slope_test.sh --no-restart       # Container restart atla
# ═══════════════════════════════════════════════════════════════════════════
...
# ── Dünya seçimi ──────────────────────────────────────────────────────────
WORLD_SDF="leo_sloped_terrain.sdf"
WORLD_NAME="leo_sloped_terrain"
WAYPOINTS="sloped_terrain_waypoints.yaml"
...
echo ""
echo -e "${BOLD}${CYAN}╔══════════════════════════════════════════════════════════╗${NC}"
echo -e "${BOLD}${CYAN}║      LEO ROVER — EĞİMLİ ARAZİ TRAVERS TESTİ            ║${NC}"
echo -e "${BOLD}${CYAN}╚══════════════════════════════════════════════════════════╝${NC}"
echo ""
...
echo ""
echo -e "${CYAN}Eğim testleri:${NC}"
echo "  • 5°  eğim (güney) — kolay"
echo "  • 10° eğim (doğu)  — orta"
echo "  • 15° eğim (güney) — zor"
echo "  • 20° eğim (batı)  — çok zor"
echo "  • 25° eğim (NE)    —极限"
echo "  • 30° eğim (SW)    —极限+"
echo ""
...
# ── 3. Waypoint testi ─────────────────────────────────────────────────────
echo ""
echo -e "${BOLD}[3/3] Eğimli arazi travers testi başlıyor...${NC}"
echo ""
echo "  Misyon planı (GPS navigasyon - eğim travers):"
echo "    WP1: Başlangıç noktası"
echo "    WP2: 5° eğim (güney)"
echo "    WP3: 10° eğim (doğu)"
echo "    WP4: 15° eğim (güney)"
echo "    WP5: 20° eğim (batı)"
echo "    WP6: Kurtarma düzlemi"
...
docker exec "$CONTAINER" bash -c "
  grep -E '(✅|⚠️|SUCCEEDED|ABORTED|REJECTED|Total|Toplam|MISSION RESULTS|FAZ|slope_test)' \
...
echo ""
echo -e "  ${CYAN}Tekrar çalıştırma: ./run_slope_test.sh${NC}"
echo -e "  ${CYAN}Restart atlayarak : ./run_slope_test.sh --no-restart${NC}"
