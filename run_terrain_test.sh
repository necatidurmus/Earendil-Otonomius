#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════════
#  run_terrain_test.sh  —  Açık Arazi Navigasyon Test Başlatıcı
#  Leo Rover: GPS navigasyon testi (tepeler, vadiler, farklı zeminler)
#
...
#    ./run_terrain_test.sh --rviz             # RViz ile görselleştir
#    ./run_terrain_test.sh --timeout 600      # 10 dakika timeout
#    ./run_terrain_test.sh --no-restart       # Container restart atla
# ═══════════════════════════════════════════════════════════════════════════
...
# ── Dünya seçimi ──────────────────────────────────────────────────────────
WORLD_SDF="leo_open_terrain.sdf"
WORLD_NAME="leo_open_terrain"
WAYPOINTS="open_terrain_waypoints.yaml"
...
echo ""
echo -e "${BOLD}${CYAN}╔══════════════════════════════════════════════════════════╗${NC}"
echo -e "${BOLD}${CYAN}║      LEO ROVER — AÇIK ARAZİ NAVİGASYON TESTİ           ║${NC}"
echo -e "${BOLD}${CYAN}╚══════════════════════════════════════════════════════════╝${NC}"
echo ""
...
# ── 3. Waypoint testi ─────────────────────────────────────────────────────
echo ""
echo -e "${BOLD}[3/3] Açık arazi waypoint testi başlıyor...${NC}"
echo ""
echo "  Misyon planı (GPS navigasyon):"
echo "    WP1: Başlangıç noktası"
echo "    WP2: Kuzey tepesi"
echo "    WP3: Doğu tepesi"
echo "    WP4: Güneydoğu"
echo "    WP5: Güneybatı"
echo "    WP6: Batı tepesi"
...
docker exec "$CONTAINER" bash -c "
  grep -E '(✅|⚠️|SUCCEEDED|ABORTED|REJECTED|Total|Toplam|MISSION RESULTS|FAZ|outdoor_gps)' \
...
echo ""
echo -e "  ${CYAN}Tekrar çalıştırma: ./run_terrain_test.sh${NC}"
echo -e "  ${CYAN}Restart atlayarak : ./run_terrain_test.sh --no-restart${NC}"
