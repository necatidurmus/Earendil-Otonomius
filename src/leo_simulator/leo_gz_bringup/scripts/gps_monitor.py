...
class GPSMonitor(Node):
    def __init__(self):
        ...
        self.declare_parameter('set_pose_service', '/ukf_filter_node/set_pose')
        self.declare_parameter('covariance_sigma_max', 10.0)
        ...
        self.covariance_sigma_max = self.get_parameter('covariance_sigma_max').value
        ...
        self.get_logger().info(f'  Startup grace  : {self._startup_grace_secs}s')
        self.get_logger().info('=' * 55)
    ...
    def _reset_ukf_global(self):
        self.get_logger().info(f'UKF reset deneniyor (servis: {self.set_pose_service})')
        self._ukf_reset_retries = 0
        self._try_ukf_reset()
    ...
    def _try_ukf_reset(self):
        ...
        self.get_logger().info(
            f'UKF Global sıfırlama isteği gönderiliyor → ({p.x:+.1f}, {p.y:+.1f})')
        
        future = self.set_pose_client.call_async(req)
        future.add_done_callback(self._ukf_reset_cb)

    def _ukf_reset_retry_cb(self):
        if self._ukf_reset_retry_timer is not None:
            self._ukf_reset_retry_timer.cancel()
            self.destroy_timer(self._ukf_reset_retry_timer)
            self._ukf_reset_retry_timer = None
        self._try_ukf_reset()

    def _ukf_reset_cb(self, future):
        try:
    ...
    def _compute_quality(self, msg: NavSatFix) -> float:
        ...
        if msg.position_covariance_type > 0 and len(cov) >= 5:
            ...
            cov_score = max(0.0, 1.0 - sigma_h / self.covariance_sigma_max)
