...
class WaypointReceiver(Node):
    def __init__(self, frame='map'):
        ...
        self.declare_parameter('frame', frame)
    ...
    def _cancel_current_goal(self):
        if self._current_goal_handle is not None:
            try:
                ...
                self.get_logger().info('Mevcut goal iptal edildi')
            except Exception as e:
    ...
    def _on_goal_response(self, future):
        try:
            ...
            self.get_logger().error(f'WP{self.current_idx + 1} gönderilemedi: {e}')
            self.results.append({
    ...
    def _on_result(self, future):
        try:
            ...
            self.get_logger().error(f'WP{self.current_idx + 1} sonuç alınamadı: {e}')
            self.results.append({
                'wp': self.current_idx + 1,
