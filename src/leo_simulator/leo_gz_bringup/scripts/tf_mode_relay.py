...
class TFModeRelay(Node):
    def __init__(self):
        ...
        self._transition_duration = self.get_parameter('transition_duration').value
        ...
        self.create_timer(1.0 / rate, self._relay_tf)

        self._latched_gps_correction = _identity_transform()

        self.get_logger().info('TF Mode Relay v0.6 baslatildi')
        self.get_logger().info(f'  TF zinciri: {self._map_frame} → {self._map_slam_frame} → {self._odom_frame} → {self._base_frame}')
    ...
    def _publish_map_map_slam(self, transform):
        ...
        relay.header.frame_id = self._map_frame
        relay.child_frame_id = self._map_slam_frame
