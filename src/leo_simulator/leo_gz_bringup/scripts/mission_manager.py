...
Kullanım:
  ros2 run leo_gz_bringup mission_manager -- --mission /path/to/hybrid_waypoints.yaml
  ya da navigation_hybrid.launch.py / run_hybrid_test.sh tarafından çağrılır.
"""
...
class MissionManager(Node):
    def __init__(self, mission_file):
        ...
        self.declare_parameter('mode_wait_timeout', 60.0)
        self.declare_parameter('slam_transition_delay', 2.0)
        self.declare_parameter('wp_timeout', 120.0)

        self._mode_wait_timeout = self.get_parameter('mode_wait_timeout').value
        self._slam_transition_delay = self.get_parameter('slam_transition_delay').value
        ...
        # GPS spoofer control — SLAM fazı için GPS_OFF, GPS fazı için AUTO
        self.spoofer_pub = self.create_publisher(String, '/gps_spoofer/control', 10)
        ...
        self.wp_start = None
        self._done = False
        self._gps_transition_pose = None
    ...
    def _start_phase(self):
        ...
        while True:
            ...
            # Spoofer kontrolü — SLAM fazı için GPS_OFF, GPS fazı için GPS_ON
            ctrl = String()
            if expected_mode == 'SLAM':
                ctrl.data = 'GPS_OFF'
                self.spoofer_pub.publish(ctrl)
                self.get_logger().info('  GPS spoofer → GPS_OFF (SLAM fazı)')
            else:
                ctrl.data = 'GPS_ON'
                self.spoofer_pub.publish(ctrl)
                self.get_logger().info('  GPS spoofer → GPS_ON (GPS fazı)')
            ...
            if self.current_mode != expected_mode:
                ...
                self._phase_mode_deadline = self.get_clock().now().nanoseconds / 1e9 + self._mode_wait_timeout
                self._phase_expected_mode = expected_mode
                self._phase_spoofer_ctrl = ctrl.data
            ...
            if expected_mode == 'SLAM':
                self.get_logger().info(f'  TF geçişi için {self._slam_transition_delay}s bekleniyor...')
                self._slam_delay_timer = self.create_timer(
                    self._slam_transition_delay, self._after_slam_delay)
                return
            self.wp_idx = 0
            self._send_wp()
    ...
    def _check_mode_transition(self):
        ...
        # Spoofer kontrolünü tekrar gönder (ilk mesaj kaçırılmış olabilir)
        ctrl = String()
        ctrl.data = self._phase_spoofer_ctrl
        self.spoofer_pub.publish(ctrl)
        ...
        if self.get_clock().now().nanoseconds / 1e9 > self._phase_mode_deadline:
            self._mode_wait_timer.cancel()
            self.destroy_timer(self._mode_wait_timer)
            # Timeout'ta spoofer'ı AUTO'ya döndür
            restore = String()
            restore.data = 'AUTO'
            self.spoofer_pub.publish(restore)
            phase = self.phases[self.phase_idx]
            name = phase.get('name', f'Phase{self.phase_idx+1}')
            wps = phase.get('waypoints', [])
    ...
    def _goal_response_cb(self, future):
        try:
            ...
            self.get_logger().error(f'  Goal gönderilemedi: {e}')
            phase = self.phases[self.phase_idx]
            wps = phase.get('waypoints', [])
    ...
    def _result_cb(self, future):
        try:
            ...
            self.get_logger().error(f'  Goal sonuç alınamadı: {e}')
            phase = self.phases[self.phase_idx]
            wps = phase.get('waypoints', [])
            name = wps[self.wp_idx].get('name', '')
            self.results.append({
                'phase': phase.get('name', ''),
                'wp': self.wp_idx + 1, 'name': name,
                'status': 'RESULT_FAILED', 'time': 0
            })
            self.wp_idx += 1
            self._send_wp()
            return

        elapsed = round((self.get_clock().now() - self.wp_start).nanoseconds / 1e9, 1)
        phase = self.phases[self.phase_idx]
        wps = phase.get('waypoints', [])
        name = wps[self.wp_idx].get('name', '')
    ...
    def _print_summary(self):
        ...
        total = round((self.get_clock().now() - self.start_time).nanoseconds / 1e9, 1)
        log = self.get_logger()
        log.info('')
        log.info('=' * 65)
