...
COVARIANCE_NO_FIX    = [9999.0, 0.0, 0.0, 0.0, 9999.0, 0.0, 0.0, 0.0, 9999.0]
...
class TunnelGPSSpoofer(Node):
    def __init__(self):
        ...
        self.declare_parameter('tunnel_exit_x_min', 19.0)
        self.declare_parameter('tunnel_exit_x_max', 31.0)
        ...
        self.t_exit_xmin = self.get_parameter('tunnel_exit_x_min').value
        self.t_exit_xmax = self.get_parameter('tunnel_exit_x_max').value
    ...
    def _get_zone(self, x: float, y: float) -> str:
        ...
        if not self.tunnel_latched:
            ...
            exited = (x < self.t_exit_xmin) or (x > self.t_exit_xmax)
            if exited:
                self.tunnel_latched = False
                self.get_logger().warn(
