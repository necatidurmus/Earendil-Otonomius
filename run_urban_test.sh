#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════════
#  run_urban_test.sh  —  Şehir/Urban Navigasyon Test Başlatıcı
#  Leo Rover: GPS navigasyon testi (binalar, yollar, kaldırım)
#
...
#    ./run_urban_test.sh --rviz             # RViz ile görselleştir
#    ./run_urban_test.sh --timeout 600      # 10 dakika timeout
#    ./run_urban_test.sh --no-restart       # Container restart atla
# ═══════════════════════════════════════════════════════════════════════════
...
# ── Dünya seçimi ──────────────────────────────────────────────────────────
WORLD_SDF="leo_urban.sdf"
WORLD_NAME="leo_urban"
WAYPOINTS="urban_waypoints.yaml"
...
echo ""
echo -e "${BOLD}${CYAN}╔══════════════════════════════════════════════════════════╗${NC}"
echo -e "${BOLD}${CYAN}║      LEO ROVER — ŞEHİR NAVİGASYON TESTİ                ║${NC}"
echo -e "${BOLD}${CYAN}╚══════════════════════════════════════════════════════════╝${NC}"
echo ""
...
# ── 3. Waypoint testi ─────────────────────────────────────────────────────
echo ""
echo -e "${BOLD}[3/3] Şehir waypoint testi başlıyor...${NC}"
echo ""
echo "  Misyon planı (GPS navigasyon - şehir blokları):"
...
docker exec "$CONTAINER" bash -c "
  grep -E '(✅|⚠️|SUCCEEDED|ABORTED|REJECTED|Total|Toplam|MISSION RESULTS|FAZ|urban_gps)' \
...
echo ""
echo -e "  ${CYAN}Tekrar çalıştırma: ./run_urban_test.sh${NC}"
echo -e "  ${CYAN}Restart atlayarak : ./run_urban_test.sh --no-restart${NC}"
