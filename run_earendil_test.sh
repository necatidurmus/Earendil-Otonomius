#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════════
#  run_earendil_test.sh  —  Earendil Environment Hibrit Test Başlatıcı
#  Leo Rover: Hibrit GPS+SLAM testi (mesh tabanlı ortam)
#
#  Orijinal kaynak: https://github.com/xlanthorbp/earendil_bot.git
#
#  Tüm ayarlar sim_config.yaml'dan okunur.
#  Argümanlar config'i override eder.
#
#  Kullanım:
#    ./run_earendil_test.sh                    # Earendil dünyası
#    ./run_earendil_test.sh --rviz             # RViz ile görselleştir
#    ./run_earendil_test.sh --timeout 600      # 10 dakika timeout
#    ./run_earendil_test.sh --no-restart       # Container restart atla
# ═══════════════════════════════════════════════════════════════════════════
...
# ── Dünya seçimi ──────────────────────────────────────────────────────────
WORLD_SDF="leo_earendil_env.sdf"
WORLD_NAME="leo_earendil_env"
WAYPOINTS="earendil_env_waypoints.yaml"
...
echo ""
echo -e "${BOLD}${CYAN}╔══════════════════════════════════════════════════════════╗${NC}"
echo -e "${BOLD}${CYAN}║      LEO ROVER — EARENDİL ENV HİBRİT TESTİ             ║${NC}"
echo -e "${BOLD}${CYAN}╚══════════════════════════════════════════════════════════╝${NC}"
echo ""
...
echo ""
echo -e "${CYAN}Kaynak: https://github.com/xlanthorbp/earendil_bot.git${NC}"
echo -e "${CYAN}Test: Hibrit GPS+SLAM (antenna, crater, lava tube)${NC}"
echo ""
...
echo -e "  ${CYAN}GPS + SLAM modu (otomatik geçiş)${NC}"
...
# ── 3. Waypoint testi ─────────────────────────────────────────────────────
echo ""
echo -e "${BOLD}[3/3] Earendil environment hibrit testi başlıyor...${NC}"
echo ""
echo "  Misyon planı (Hibrit GPS+SLAM):"
echo "    ── FAZ 1: earendil_gps_1 (GPS modu) ──"
echo "    WP1: Başlangıç noktası"
echo "    WP2: Antenna"
echo "    WP3: Krater yaklaşımı"
echo "    ── FAZ 2: earendil_slam_tunnel (SLAM modu) ──"
echo "    WP4: Lava tüneli girişi"
echo "    WP5: Lava tüneli çıkışı"
echo "    ── FAZ 3: earendil_gps_2 (GPS modu) ──"
echo "    WP6: Dönüş yolu"
...
docker exec "$CONTAINER" bash -c "
  grep -E '(✅|⚠️|SUCCEEDED|ABORTED|REJECTED|Total|Toplam|MISSION RESULTS|FAZ|earendil_gps|earendil_slam)' \
...
echo ""
echo -e "  ${CYAN}Tekrar çalıştırma: ./run_earendil_test.sh${NC}"
echo -e "  ${CYAN}Restart atlayarak : ./run_earendil_test.sh --no-restart${NC}"
