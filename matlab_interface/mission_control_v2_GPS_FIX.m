%% 
classdef mission_control_v2_GPS_FIX < matlab.apps.AppBase
    % ╔══════════════════════════════════════════════════════════════╗
    ...
    methods (Access = private)
        ...
        function setupDirs(app)
            ...
            app.MissionsDir = fullfile(getenv('HOME'), 'zeynep_missions');
        ...
        function buildObstacleDB(app)
            ...
            db.industrial.boxes = [ ...
                -8,  5, 0.5, 3.0;  -8, -5, 0.5, 3.0; ...
                 0,  5, 0.5, 3.0;   0, -5, 0.5, 3.0; ...
                 8,  5, 0.5, 3.0;   8, -5, 0.5, 3.0];
            db.industrial.tunnel = [];
            db.industrial.spawn  = [0, 0];
            db.industrial.xlim   = [-15 15];
            db.industrial.ylim   = [-15 15];
        ...
        function onOdomCallback(app, msg)
            ...
            if ~isempty(app.TFBuffer)
                try
                    ...
                    % Cache the offset between global GPS map and local odometry
                    app.MapToOdomOffsetX = x_map - msg.pose.pose.position.x;
                    app.MapToOdomOffsetY = y_map - msg.pose.pose.position.y;
                catch
                end
            end
            
            if isempty(x_map)
                % If TF drops, use local odometry + cached offset to maintain GPS coordinates
                x = msg.pose.pose.position.x + app.MapToOdomOffsetX;
                y = msg.pose.pose.position.y + app.MapToOdomOffsetY;
            else
                x = x_map;
                y = y_map;
            end
    ...
    methods (Access = private)
        ...
        function loadMissionLibrary(app)
            ...
            missionsDir = fullfile(getenv('HOME'), 'zeynep_missions');
        ...
        function onEStop(app, ~)
            ...
            if ~app.OfflineMode
                ...
                % 1. Robotun ŞU ANKİ konumunu (map) hedef ver → Nav2 "vardım" der.
                try
                    odom = receive(app.OdomSub, 0.5);
            ...
            uialert(app.UIFigure, ...
                ['🛑 EMERGENCY STOP AKTİF!' newline newline ...
                 'Robot durduruldu, Emergency Stop''a basıldı.' newline ...
        ...
        function onReset(app, ~)
            ...
            % Ask user if they want to resume the mission
            if app.CurrentWaypointIdx <= size(app.WaypointTable.Data, 1) && app.CurrentWaypointIdx > 1
                sel = uiconfirm(app.UIFigure, 'Emergency Stop kaldirildi. Goreve kaldigi yerden (Siradaki Waypoint) devam edilsin mi?', 'Resume Mission', ...
                    'Options', {'Evet (Devam Et)', 'Hayir (Gorevi Iptal Et)'}, 'DefaultOption', 1, 'CancelOption', 2);
                
                if strcmp(sel, 'Evet (Devam Et)')
                    app.IsNavigating = true;
                    app.MissionTimeLbl.Text = 'Mission Time: Resumed...';
                    app.MissionTimeLbl.FontColor = 'blue';
                    app.sendNextWaypoint();
                    return;
                end
            ...
            % If not resuming, reset everything completely
            app.IsNavigating = false;
            app.CurrentWaypointIdx = 1;
        ...
        function onGeneratePDF(app, ~)
            sel = app.HistoryTable.Selection;
            if isempty(sel)
                uialert(app.UIFigure, 'Select a result to generate PDF!', 'No Selection');
                return;
            end
            row = sel(1);
            name = app.HistoryTable.Data{row, 1};
            ...
            try
                ...
                logData = d.data.log; % [Timestamp, Velocity, X, Y]
                wpData = d.data.waypoints;
                
                % Ask for save location
                [file, path] = uiputfile('*.pdf', 'Save PDF Report', [name '_Report.pdf']);
                ...
                % Title
                sgtitle(fig, sprintf('Mission Report: %s', strrep(name, '_', '\_')), 'FontSize', 16, 'FontWeight', 'bold');
                
                % Subplot 1: Map and Route
                ax1 = subplot(2, 2, [1 3], 'Parent', fig);
                hold(ax1, 'on');
                grid(ax1, 'on');
                title(ax1, 'Trajectory Analysis');
                xlabel(ax1, 'X (m)'); ylabel(ax1, 'Y (m)');
                
                % Plot actual path
                if ~isempty(logData)
                    plot(ax1, logData(:,3), logData(:,4), 'Color', [0.2 0.4 0.8], 'LineWidth', 2);
                    plot(ax1, logData(1,3), logData(1,4), 'go', 'MarkerFaceColor', 'g', 'MarkerSize', 8);
                    plot(ax1, logData(end,3), logData(end,4), 'ro', 'MarkerFaceColor', 'r', 'MarkerSize', 8);
                end
                
                % Plot waypoints
                if ~isempty(wpData)
                    plot(ax1, wpData(:,1), wpData(:,2), 'ks', 'MarkerSize', 8, 'LineWidth', 1.5);
                    for i = 1:size(wpData,1)
                        text(ax1, wpData(i,1)+0.5, wpData(i,2)+0.5, sprintf('WP%d', i), 'Color', 'k', 'FontSize', 9);
                    end
                end
                axis(ax1, 'equal');
                ...
                if ~isempty(logData)
                    t0 = logData(1,1);
                    plot(ax2, logData(:,1)-t0, logData(:,2), 'r-', 'LineWidth', 1.5);
                    xlabel(ax2, 'Time (s)'); ylabel(ax2, 'Velocity (m/s)');
                    title(ax2, 'Velocity Profile');
                    grid(ax2, 'on');
                end
                
                % Subplot 3: Statistics Text
                ax3 = subplot(2, 2, 4, 'Parent', fig);
                axis(ax3, 'off');
                
                if ~isempty(logData)
                    duration = logData(end,1) - logData(1,1);
                    avgVel = mean(abs(logData(:,2)));
                    maxVel = max(abs(logData(:,2)));
                    
                    dist = 0;
                    for i=2:size(logData,1)
                        dist = dist + hypot(logData(i,3)-logData(i-1,3), logData(i,4)-logData(i-1,4));
                    end
                    
                    stats = {
                        'MISSION METRICS:'
                        '----------------------------'
                        sprintf('Duration         : %.1f sec', duration)
                        sprintf('Total Distance   : %.2f m', dist)
                        sprintf('Average Velocity : %.2f m/s', avgVel)
                        sprintf('Maximum Velocity : %.2f m/s', maxVel)
                        sprintf('Waypoints Given  : %d', size(wpData,1))
                        sprintf('Safe Clearance   : %.2f m', d.data.params.safe_dist)
                        sprintf('Goal Tolerance   : %.2f m', d.data.params.goal_tol)
                        '----------------------------'
                    };
                    text(ax3, 0.1, 0.5, stats, 'FontSize', 12, 'FontName', 'Courier', 'FontWeight', 'bold');
                end
                ...
                uialert(app.UIFigure, 'PDF report generated and saved successfully!', 'Report Ready', 'Icon', 'success');
            catch ME
                uialert(app.UIFigure, ['Failed to generate PDF: ' ME.message], 'Error', 'Icon', 'error');
            end
        end
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            app.UIFigure.Name = 'MATLAB-Based Operator Interface';
    ...
    methods (Access = public)
        ...
        function app = mission_control_v2_GPS_FIX
