...
classdef mission_control_v2_DENEME < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        function onStartMission(app, ~)
            ...
            % Initialize tracking states — robotun MEVCUT konumunu başlangıç olarak kullan
            app.LastPosition = [app.RobotX, app.RobotY];
            app.LastProgressTime = tic;
            ...
            % ═══ ROBOT MEVCUT KONUM KONTROLÜ ═══
            % Robot zaten WP1'e yakınsa, WP1'i atla (önceki mission'dan kalmış olabilir)
            total_wp = size(app.WaypointTable.Data, 1);
            goal_tol = app.GoalTolField.Value;
            wp1_x = app.WaypointTable.Data(1, 1);
            wp1_y = app.WaypointTable.Data(1, 2);
            dist_to_wp1 = sqrt((app.RobotX - wp1_x)^2 + (app.RobotY - wp1_y)^2);

            if dist_to_wp1 <= goal_tol + 0.5
                fprintf('🤖 Robot zaten WP1''e yakın (%.2fm), WP1 atlanıyor.\n', dist_to_wp1);
                app.CurrentWaypointIdx = 2;
                if total_wp >= 2
                    app.sendNextWaypoint();
                else
                    % Tek WP var ve zaten oradayız — mission tamamlandı
                    app.IsNavigating = false;
                    app.ProgressGauge.Value = 100;
                    app.CurrentWPLbl.Text = 'WP 1/1 ✓';
                    app.MissionTimeLbl.Text = '✓ Already at waypoint!';
                    app.MissionTimeLbl.FontColor = 'green';
                    return;
                end
            else
